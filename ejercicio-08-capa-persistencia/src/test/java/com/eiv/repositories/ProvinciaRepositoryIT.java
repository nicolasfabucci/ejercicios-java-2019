...
public class ProvinciaRepositoryIT {
    ...
    @Test
    @Transactional
    public void givenProvincia_whenFindById_thenDelete() {

        ProvinciaRepository provinciaRepository = new ProvinciaRepository(dataSource);
        
        provinciaRepository.findById(5L).ifPresent(provinciaEntity -> {
            
            provinciaRepository.delete(provinciaEntity);
        });
        
        Optional<ProvinciaEntity> optional = provinciaRepository.findById(5L);
